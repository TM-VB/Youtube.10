...
object YtDlpErrorMapper {
    ...
    fun map(throwable: Throwable): DownloadError {
        ...
        return when {
            throwable is UnknownHostException || throwable is ConnectException || message.contains("network") || message.contains("connection") || message.contains("failed to connect") -> {
            ...
            message.contains("sign in") || message.contains("login") || message.contains("confirm your age") || message.contains("bot") -> {
            ...
            message.contains("unavailable") || message.contains("video is unavailable") || message.contains("not found") || message.contains("deleted") -> {
                DownloadError.VideoUnavailable(
                    msg = "This video is unavailable or has been removed.",
                    detail = throwable.message
                )
            }
            message.contains("unsupported url") || message.contains("not a valid url") || message.contains("invalid url") -> {
                DownloadError.InvalidUrl(
                    msg = "Invalid or unsupported video URL.",
                    detail = throwable.message
                )
            }
            message.contains("no format") || message.contains("requested format not available") -> {
